...
def main() -> None:
    ...
    R = ask_level(
        "Risk tolerance / volatility comfort\n"
        "Meaning: How comfortable you are with your investments moving up and down in value.\n"
        "1. Risk tolerance / volatility comfort"
    )

    H = ask_level(
        "Investment horizon\n"
        "Meaning: How long you plan to keep your money invested before needing it.\n"
        "2. Investment horizon"
    )

    D = ask_level(
        "Diversification preference\n"
        "Meaning: Whether you prefer spreading money across many assets or concentrating in fewer.\n"
        "3. Diversification preference"
    )

    E = ask_level(
        "Ethical / ESG investment priority\n"
        "Meaning: How important it is that investments align with ethical or sustainability values.\n"
        "4. Ethical / ESG investment priority"
    )

    S = ask_level(
        "Sector / industry exposure preference\n"
        "Meaning: Whether you want targeted exposure to specific sectors (e.g., tech, healthcare).\n"
        "5. Sector / industry exposure preference"
    )

    T = ask_level(
        "Portfolio turnover preference\n"
        "Meaning: Whether you prefer a portfolio that trades frequently or stays mostly stable.\n"
        "6. Portfolio turnover preference"
    )
    ...
    # --------------------------------------------------
    # AUTO-LAUNCH DASHBOARD (ROBUST, macOS FRIENDLY)
    # --------------------------------------------------

    project_root = os.path.dirname(os.path.abspath(__file__))

    # Write selected Robo so dashboard auto-focuses
    with open(os.path.join(project_root, ".selected_robo.txt"), "w") as f:
        f.write(folder)

    dashboard_path = os.path.join(project_root, "dashboard.py")

    if not os.path.exists(dashboard_path):
        print("dashboard.py not found — skipping dashboard launch.")
        return
    ...
    # Open browser explicitly (macOS)
    subprocess.Popen(["open", "http://127.0.0.1:8501"])

    print("Dashboard launched at http://127.0.0.1:8501")
